...
- (instancetype)initWithSettings:(NSDictionary *)settings
  ...
  if (self = [super init]) {
    ...
    // For Segment React Native. We should call our applicationDidBecomeActive
    // in case we were initialized too late and missed the first launch
    dispatch_async(dispatch_get_main_queue(), ^{
      BOOL alreadyActive = [[UIApplication sharedApplication]
                               applicationState] == UIApplicationStateActive;
      // for regular Segment integration alreadyActive should always be false
      if (alreadyActive) {
        [self applicationDidBecomeActive];
        NSLog(@"Segment React Native AppsFlye rintegration is used, sending "
...
- (instancetype)initWithSettings:(NSDictionary *)settings
                   withAppsflyer:(AppsFlyerTracker *)aAppsflyer {
...
- (void)track:(SEGTrackPayload *)payload {
  ...
  // Extract the revenue / currency from the properties passed in to us with an
  // "af_" prefix
  NSNumber *revenue = [SEGAppsFlyerIntegration extractRevenue:payload.properties
                                                      withKey:@"revenue"];
  NSString *currency =
      [SEGAppsFlyerIntegration extractCurrency:payload.properties
                                       withKey:@"currency"];

  if (revenue) {
    NSMutableDictionary *af_payload_properties =
        [NSMutableDictionary dictionaryWithDictionary:payload.properties];
...
+ (NSString *)extractCurrency:(NSDictionary *)dictionary
                      withKey:(NSString *)currencyKey {
...
+ (NSDecimalNumber *)extractRevenue:(NSDictionary *)dictionary
                            withKey:(NSString *)revenueKey {
...
- (void)onConversionDataSuccess:(nonnull NSDictionary *)conversionInfo {
  ...
  if (!installAttrSent) {
    ...
    if (_segDelegate &&
        [_segDelegate respondsToSelector:@selector(onConversionDataSuccess:)]) {
      //   [_segDelegate onConversionDataSuccess:conversionInfo];
    }
    NSDictionary *campaign = @{
      @"source" :
          [SEGAppsFlyerIntegration validateNil:conversionInfo[@"media_source"]],
      @"name" :
          [SEGAppsFlyerIntegration validateNil:conversionInfo[@"campaign"]],
      @"ad_group" :
          [SEGAppsFlyerIntegration validateNil:conversionInfo[@"adgroup"]]
    };

    NSMutableDictionary *properties = [NSMutableDictionary
        dictionaryWithDictionary:@{@"provider" : @"AppsFlyer"}];
    ...
    // If you are working with networks that don't allow passing user level data
    // to 3rd parties, you will need to apply code to filter out these networks
    // before calling
...
- (void)onConversionDataFail:(nonnull NSError *)error {
  if (_segDelegate &&
      [_segDelegate respondsToSelector:@selector(onConversionDataFail:)]) {
...
- (void)onAppOpenAttribution:(NSDictionary *)attributionData {
  if (_segDelegate &&
      [_segDelegate respondsToSelector:@selector(onAppOpenAttribution:)]) {
...
- (void)onAppOpenAttributionFailure:(NSError *)error {
  if (_segDelegate && [_segDelegate respondsToSelector:@selector
...
- (BOOL)trackAttributionData {
  return [(NSNumber *)[self.settings objectForKey:@"trackAttributionData"]
      boolValue];
}
