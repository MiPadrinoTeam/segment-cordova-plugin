...
- (void)screen:(SEGScreenPayload *)payload {
  [[[UAirship shared] analytics]
      trackScreen:(payload.name ?: payload.category)];
}
...
- (void)track:(SEGTrackPayload *)payload {
  ...
  NSNumber *value =
      [SEGUrbanAirshipIntegration extractRevenue:payload.properties
                                         withKey:@"revenue"];
  if (!value) {
    value = [SEGUrbanAirshipIntegration extractRevenue:payload.properties
                                               withKey:@"value"];
  }

  if (value) {
    customEvent.eventValue =
        [NSDecimalNumber decimalNumberWithDecimal:[value decimalValue]];
  }
